...
_Use `-help` to see all available options for each executable._

Your node is now running and starts with a _synchronization process_ that downloads the entire blockchain from other nodes. This operation will take many hours to complete, but you are now part of the Dogecoin network!
...
To receive DOGE, you need an address that is securely derived from a private key through a series of automatic, cryptographic operations. The _address_ can be shared with anyone to receive DOGE, but the _private key_ is sensitive information that allows anyone that knows it to spend the DOGE on the associated address.
...
Blocks and transactions are identified by unique _hashes_.
Let's find the _[coinbase transaction](https://www.javatpoint.com/coinbase-transaction)_ of block 69.
...
When not specifying any network, _Mainnet_ is the network used by default. To enable _testnet_, use the `dogecoind -testnet`.

To enable _regtest_, use the `-regtest` option.
