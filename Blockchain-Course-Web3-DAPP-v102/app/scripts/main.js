...
let filterWatch
let filterEventCounter

// Holds the contract event object
let contractEvent
let contractEventCounter = 0
...
window.addEventListener('load', function () {
  ...
  if (typeof web3 !== 'undefined') {
    ...
    const provider = document.getElementById('provider_url').value
...
function doGetBalances (accounts) {
  ...
  for (var i = 0; i < accounts.length; i++) {
    ...
    web3.eth.getBalance(accounts[i], web3.eth.defaultBlock, function (error, result) {
      ...
      const bal = web3.fromWei(result, 'ether').toFixed(2)
...
function doSendTransaction () {
  const transactionObject = createTransactionObjectJson()
  ...
  web3.eth.sendTransaction(transactionObject, function (error, result) {
    if (error) {
      ...
      const etherscanLinkA = document.getElementById('etherscan_io_tx_link')
...
function doUnlockAccount () {
  ...
  const account = document.getElementById('select_to_unlock_account').value
  const password = document.getElementById('unlock_account_password').value
...
function doLockAccount () {
  ...
  web3.personal.lockAccount(account, function (error, result) {
    ...
    if (error) {
      ...
      const str = account.substring(0, 20) + '...Locked'
      setData('lock_unlock_result', str, false)
    }
  })
}
...
function doDeployContract () {
  ...
  const abiDefinitionString = document.getElementById('compiled_abidefinition').value
  const abiDefinition = JSON.parse(abiDefinitionString)

  const bytecode = document.getElementById('compiled_bytecode').value

  // 1. Create the contract object
  const contract = web3.eth.contract(abiDefinition)

  // Get the estimated gas
  const gas = document.getElementById('deployment_estimatedgas').value
...
function doDeployContractSynchronous () {
  ...
  const contractData = contract.new.getData(10, { data: bytecode })
  console.log('Contract Data=', contractData)
  // call send transaction and then call getTransactionReceipt
  params.data = contractData
  const transactionHash = web3.eth.sendTransaction(params)
...
function doContractFunctionCall () {
  ...
  const funcName = document.getElementById('contract_select_function').value

  if (funcName === 'setNum') {
    const parameterValue = document.getElementById('setnum_parameter').value

    // MetaMask does not allow synchronous call to 'call' for non-constant function
    // Change this to asynchronous :)
    const value = instance.setNum.call(parameterValue)
...
function doContractSendCall () {
  ...
  const estimatedGas = document.getElementById('contract_execute_estimatedgas').value
  const parameterValue = document.getElementById('setnum_parameter').value
  const funcName = document.getElementById('contract_select_function').value
...
function doFilterGetLogs () {
  ...
  const filterGet = web3.eth.filter(options)
...
function doContractEventGet () {
  ...
  const event = createContractEventInstance()
...
function createContractEventInstance () {
  const contractAddress = document.getElementById('contract_instance_address').value

  const contractInstance = createContractInstance(contractAddress)

  // geth the indexed data values JSON
  let indexedEventValues = document.getElementById('indexed_event_values').value
  indexedEventValues = JSON.parse(indexedEventValues)

  let additionalFilterOptions = document.getElementById('additional_filter_event_values').value
